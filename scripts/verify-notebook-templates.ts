...
const blank = createNotebookFromTemplate("blank", "ja");
assert.equal(blank.children.length, 0, "Blank template should have no user nodes.");

const daily = createNotebookFromTemplate("daily-notes", "ja");
...
const scamper = createNotebookFromTemplate("scamper", "ja");
assert.ok(child(scamper, "Substitute - 置き換える"));
assert.ok(child(scamper, "Reverse - 逆にする"));
assert.ok(child(scamper, "次に試すこと"));

const englishTravel = createNotebookFromTemplate("travel", "en");
assert.deepEqual(titles(englishTravel), ["Things to do", "Itinerary", "Packing", "Reservations and tickets"]);
assert.ok(child(child(englishTravel, "Itinerary"), "Day 1"));
